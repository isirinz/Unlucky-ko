    ...
    public void handleBattleEvent(BattleEvent event) {
        switch (event) {
            ...
            case ENEMY_TURN:
                ...
                String[] dialog = battle.enemyTurn(rm);
        ...
        if (player.isDead()) {
            ...
            if (Util.isSuccess(Util.REVIVAL)) {
                startDialog(new String[] {
                        rm.bundle.get("PLAYER_DAMAGE_REVIVAL_AFTER_DIED1"),
                        rm.bundle.get("PLAYER_DAMAGE_REVIVAL_AFTER_DIED2")
                ...
                startDialog(new String[] {
                        rm.bundle.get("PLAYER_DAMAGE_DIED1"),
                        rm.bundle.format("PLAYER_DAMAGE_DIED2", Util.DEATH_PENALTY)
    ...
    private boolean applyEnemyDamage() {
        ...
        if (battle.opponent.isDead()) {
            ...
            if (Util.isSuccess(Util.REVIVAL) && !battle.opponent.isBoss()) {
                startDialog(new String[] {
                        rm.bundle.get("ENEMY_DAMAGE_DIED1"),
                        rm.bundle.get("ENEMY_DAMAGE_DIED2")
            ...
            // maybe the player gets an item
                ...
                if (player.getExp() + expGained >= player.getMaxExp()) {
                    ...
                    startDialog(new String[] {
                            rm.bundle.format("You_defeated", battle.opponent.getId()),
                            rm.bundle.format("You_obtained_gold", goldGained),
                            battle.getItemDialog(rm, itemGained),
                            rm.bundle.format("You_gained_experience",expGained),
                            rm.bundle.get("You_leveled_up")
        ...
        if (battle.opponent.isBoss()) {
            ...
            if (b.bossId == 0) {
                if (battle.opponent.numRespawn + 1 < 4) {
                    ...
                    startDialog(new String[] {
                            rm.bundle.get("BOSS_DEATH1"),
                            rm.bundle.format("BOSS_DEATH2", (3 - battle.opponent.numRespawn))
                    }, BattleEvent.ENEMY_TURN, BattleEvent.ENEMY_TURN);
                    return true;
                }
            }
